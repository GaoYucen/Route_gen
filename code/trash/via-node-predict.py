...
class ViaNodePredictionModel(nn.Module):
    ...
    def forward(self, s, t, s_partition, t_partition, via_node):
        ...
        # s_emb = torch.from_numpy(node_embeddings[s]).unsqueeze(0)
        # t_emb = torch.from_numpy(node_embeddings[t]).unsqueeze(0)
        # via_node_emb = torch.from_numpy(node_embeddings[via_node]).unsqueeze(0)
