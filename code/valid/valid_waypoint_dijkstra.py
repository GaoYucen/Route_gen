...
#%% 读取waypoint_dijkstra的中间点
with open('data/'+city_name+'/mid_point_list_dijkstra.pkl', 'rb') as f:
    mid_point_list = pickle.load(f)
    f.close()

#%% 构造新数据集，将每一段轨迹分为两段，第一段是起点到中间点，第二段是中间点到终点
new_test_data = []
new_test_data_2 = []
for i, trip in enumerate(test_data):
    new_test_data.append([trip[0], mid_point_list[i]])
    new_test_data.append([mid_point_list[i], trip[-1]])
    mid = len(trip) // 2
    new_test_data_2.append(trip[:mid])
    new_test_data_2.append(trip[mid:])
test_data = new_test_data_2
...
with torch.no_grad():  # 不需要梯度计算，提高速度并减少内存消耗
    for i in tqdm(range(0, len(test_data), batch_size)):
        ...
        all_waypoint_paths = [item for item in new_test_data[i:i + batch_size]]
        gens = [[t[0]] for t in all_waypoint_paths] # 起点
        ...
        for _ in range(MAX_ITERS):
            ...
            for identity, choice_tmp in zip(pending_trip_ids, chosen):
                ...
                if choice == all_waypoint_paths[identity][-1] or haversine(node_df[node_df['osmid'] == choice][['y', 'x']].values[0], node_df[node_df['osmid'] == all_waypoint_paths[identity][-1]][['y', 'x']].values[0])*1000 < 500:
                    del pending[identity]
                    continue
        ...
        # 将每两段轨迹合成一段，加入到predicted_paths和original_paths中
        for i in range(0, len(all_paths), 2):
            predicted_paths.append(gens[i] + gens[i+1])
            original_paths.append(all_paths[i] + all_paths[i+1])
        for i in range(0, len(arrival_list), 2):
            if i in arrival_list and i+1 in arrival_list:
                arrival_predicted_paths.append(gens[i] + gens[i+1])
                arrival_original_paths.append(all_paths[i] + all_paths[i+1])


#%% 计算抵达率
print('reach_num:', len(arrival_predicted_paths))
...
print('reach rate:', len(arrival_predicted_paths) / len(original_paths))
