...
- graph_sc.pkl：强连通图
- [train, valid, test]_data_small_sc：训练，测试，验证集的小规模集合，数据格式是点序列
...
- preprocess: 数据预处理（我们假设以节点node为关注点）：
  - data_preprocess_for_scc.py：计算强连通图
  - data_preprocess_for_traj.py & data_preprocess_for_traj_small.py：将边序列转为点序列
...
- via-node-predict.py: 预测途经点的MLP模型并验证效果 性能：
   - 准确率：83.77%
   - 预测出途经点在轨迹上的比例：0.6
- via_node_predict_regular.py: 预测途经点的MLP模型并验证效果（正则化）
  - 增强正则化
    - 提高 Dropout 率到 0.3 
    - 在每个主要层后添加 LayerNorm 
