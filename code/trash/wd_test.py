...
file_name = 'NeuroMLR/preprocessed_data/'+city_name+'/preprocessed_train_trips_small.pkl'
...
for traj in trajs_all:
    start_end_count[(traj[0], traj[-1])] = start_end_count.get((traj[0], traj[-1]), 0) + 1
start_end_count = sorted(start_end_count.items(), key=lambda x: x[1], reverse=True)

#%% 取出最后5%的起终点对形成list
start_end_list = [i[0] for i in start_end_count[int(len(start_end_count)*0.98):]]

#%% 计算不包含start_end_list的trajs
trajs = []
trajs_2 = []
for traj in trajs_all:
    if (traj[0], traj[-1]) not in start_end_list:
        trajs.append(traj)
    else:
        trajs_2.append(traj)

#%% 最长的traj
print(max([len(i) for i in trajs_all]))

#%% 统计trajs中有多少个不同的数字
links = set([j for i in trajs_all for j in i])
print(len(links))
...
node_df = gpd.read_file('NeuroMLR/preprocessed_data/'+city_name+'/map/nodes.shp')
edge_df = gpd.read_file('NeuroMLR/preprocessed_data/'+city_name+'/map/edges.shp')
...
for traj in trajs_2:
    ...
    # while len(link_topk) < 100:
    #     link_topk.append((0, 0))
    label = [0 for i in range(k)]
    for i, link in enumerate(link_topk):
        if link[0] in traj:
            label[i] = 1
    for i in range(k):
        data_2.append((start, end, link_topk[i][0], label[i]))
...
class MLP(nn.Module):
    ...
    def predict(self, data, batch_size):
        ...
        for i in tqdm(range(0, len(data), batch_size)):
            batch_x, batch_y = get_batch(i, i + batch_size, data)
        ...
        # for i in tqdm(range(0, 1000, batch_size)):
            ...
            for j in range(len(batch_y)):
                if y_pred[j] > 0.5 and batch_y[j] == 1:
                    ...
                    score_2 += 1/k
        return score, max_cnt, score_2, mid_point_list
    #
...
torch.save(mlp.state_dict(), 'new_code/param/chengdu_3_mlp_'+str(trajs_num)+'_'+str(k)+'.pth')

#%% 读取模型参数
mlp = MLP(3 * max_link, 100, 1).to(device)
mlp.load_state_dict(torch.load('new_code/param/chengdu_3_mlp_'+str(trajs_num)+'_'+str(k)+'.pth'))

#%%
print('start predict')
score, max_cnt, score_2, mid_point_list_unpopular = mlp.predict_2(data_2, batch_size_tmp)
...
with open('new_code/mid_point_list_unpopular.txt', 'w') as f:
    for item in mid_point_list_unpopular:
        f.write(str(item)+'\n')
    f.close()

# #%% 读取mid_point_list到np.array
# mid_point_list = []
# with open('new_code/mid_point_list.txt', 'r') as f:
...
#
# #%% 提取edge_df.iloc[0]中的geometry，并读取LINESTRING (xx, xx)中的第一个元素和最后一个元素
# from shapely.geometry import LineString
# coords_list = list(edge_df.iloc[0]['geometry'].coords)
#
# #%% 查看3252是否在edge_df的osmid中
# print(3252 in edge_df['osmid'])