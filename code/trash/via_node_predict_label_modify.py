...
graph_path = r"Bidijkstra\data\harbin_data\graph_sc.pkl"
...
def get_train_data(city_name, num_samples):               # 读取candidate_list和on_traj_flag_list数据
    ...
    for i in range(num_samples):
        ...
        for candidate, on_traj_flag in zip(candidate_list[i], on_traj_flag_list[i]):
            ...
            # if label == 1 :
            #     distance_source_to_via = nx.dijkstra_path_length(G, source, via_node)
            #
            #     # 计算从 via_node 到 target 的最短路径
            #     distance_via_to_target = nx.dijkstra_path_length(G, via_node, target)
            #
            #     # 最终的最短路径之和
            #     total_distance = distance_source_to_via + distance_via_to_target
            #
            #     label = torch.exp(torch.tensor(-total_distance/1000))
